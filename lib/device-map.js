...
// seems to be a timing issue - config is self modifying when it loads. 
// Maybe because config folder is mounted? Defer require until needed.
...
const self = module.exports = {
    ...
    */
    advertise: function(req, res) {
        ...
        // deferred config load - see above.
        config = require('../lib/config.js');

        var meta = req.body;
        const uid = meta.deviceID;
        ...
        if (devices[uid] != undefined) {
            ...
            // New device
            let dev = {};
            meta.mudURL = config.mudServer() + '/' + meta.modelUID64 + '.json';
            dev.device = meta;
    ...
    // Device canceled advertisement
    cancel: function(req, res) {
        let UID = req.body.deviceID;
        if (UID == undefined) {
            res.status(400).end();
            return;
        }
            ...
            if (dev == undefined) {
                ...
                dev.state = PAIR_STATE_CANCELED;
                let session = sessions[UID];
                session.res.status(204).end();
                delete sessions[UID];
            }            
        }
    },
    ...
    pairDevice: function(UID, token, subscriberID, callback) {
        (async () => {
            ...
            try {
                ...
                var uri = config.msoPortalUrl()+"/portal/v1/ca/csrt";
                console.log("pairDevice uri: "+uri);
    ...
    requestCert: function(req, res) {
        ...
        (async () => {
            ...
            let dev = devices[req.body.deviceID];
            let http_status = 500;  // Default error status

            try {
                if (dev == undefined ) {
                    http_status = 410;
                    throw "Device not found: "+req.body.deviceID;
                }
                ...
                var uri = config.msoPortalUrl()+"/portal/v1/ca/cert";
